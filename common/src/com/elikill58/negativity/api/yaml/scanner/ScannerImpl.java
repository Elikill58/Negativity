...
import com.elikill58.negativity.api.yaml.tokens.AliasToken;
import com.elikill58.negativity.api.yaml.tokens.AnchorToken;
import com.elikill58.negativity.api.yaml.tokens.BlockEndToken;
import com.elikill58.negativity.api.yaml.tokens.BlockEntryToken;
import com.elikill58.negativity.api.yaml.tokens.BlockMappingStartToken;
import com.elikill58.negativity.api.yaml.tokens.BlockSequenceStartToken;
import com.elikill58.negativity.api.yaml.tokens.DirectiveToken;
import com.elikill58.negativity.api.yaml.tokens.DocumentEndToken;
import com.elikill58.negativity.api.yaml.tokens.DocumentStartToken;
import com.elikill58.negativity.api.yaml.tokens.FlowEntryToken;
import com.elikill58.negativity.api.yaml.tokens.FlowMappingEndToken;
import com.elikill58.negativity.api.yaml.tokens.FlowMappingStartToken;
import com.elikill58.negativity.api.yaml.tokens.FlowSequenceEndToken;
import com.elikill58.negativity.api.yaml.tokens.FlowSequenceStartToken;
import com.elikill58.negativity.api.yaml.tokens.KeyToken;
import com.elikill58.negativity.api.yaml.tokens.ScalarToken;
import com.elikill58.negativity.api.yaml.tokens.StreamEndToken;
import com.elikill58.negativity.api.yaml.tokens.StreamStartToken;
import com.elikill58.negativity.api.yaml.tokens.TagToken;
import com.elikill58.negativity.api.yaml.tokens.TagTuple;
import com.elikill58.negativity.api.yaml.tokens.Token;
import com.elikill58.negativity.api.yaml.tokens.ValueToken;
import com.elikill58.negativity.api.yaml.util.ArrayStack;
...
public final class ScannerImpl implements Scanner {
	...
	private void fetchMoreTokens() {
		...
		final String text = String.format(
				"found character '%s' that cannot start any token. (Do not use %s for indentation)", chRepresentation,
				chRepresentation);
				...
				if (key.getLine() != this.reader.getLine() || this.reader.getIndex() - key.getIndex() > 1024) {
					if (key.isRequired()) {
						throw new ScannerException("while scanning a simple key", key.getMark(),
								"could not find expected ':'", this.reader.getMark());
					}
					iterator.remove();
				}
			}
	...
	private void savePossibleSimpleKey() {
		...
		if (this.allowSimpleKey) {
			...
			final SimpleKey key = new SimpleKey(tokenNumber, required, this.reader.getIndex(), this.reader.getLine(),
	...
	private boolean checkPlain() {
		...
		return Constant.NULL_BL_T_LINEBR.hasNo(c, "-?:,[]{}#&*!|>'\"%@`")
				|| (Constant.NULL_BL_T_LINEBR.hasNo(this.reader.peek(1))
						&& (c == 45 || (this.flowLevel == 0 && "?:".indexOf(c) != -1)));
	}
	...
	private String scanDirectiveName(final Mark startMark) {
		...
		if (length == 0) {
			...
			throw new ScannerException("while scanning a directive", startMark,
					"expected alphabetic or numeric character, but found " + s + "(" + c + ")", this.reader.getMark());
		}
		...
		if (Constant.NULL_BL_LINEBR.hasNo(c)) {
			...
			throw new ScannerException("while scanning a directive", startMark,
					"expected alphabetic or numeric character, but found " + s2 + "(" + c + ")", this.reader.getMark());
		}
		return value;
	}
	...
	private List<Integer> scanYamlDirectiveValue(final Mark startMark) {
		...
		if (c != 46) {
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark,
					"expected a digit or '.', but found " + s + "(" + c + ")", this.reader.getMark());
		}
		this.reader.forward();
		...
		if (Constant.NULL_BL_LINEBR.hasNo(c)) {
			final String s2 = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark,
					"expected a digit or ' ', but found " + s2 + "(" + c + ")", this.reader.getMark());
		}
	...
	private Integer scanYamlDirectiveNumber(final Mark startMark) {
		...
		if (!Character.isDigit(c)) {
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark,
					"expected a digit, but found " + s + "(" + c + ")", this.reader.getMark());
		}
		int length;
	...
	private String scanTagDirectiveHandle(final Mark startMark) {
		...
		if (c != 32) {
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark,
					"expected ' ', but found " + s + "(" + c + ")", this.reader.getMark());
		}
		return value;
	}
	...
	private String scanDirectiveIgnoredLine(final Mark startMark) {
		...
		if (lineBreak.length() == 0 && c != 0) {
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a directive", startMark,
					"expected a comment or a line break, but found " + s + "(" + c + ")", this.reader.getMark());
		}
		return lineBreak;
	}
			...
			if (Character.isDigit(c)) {
				...
				if (increment == 0) {
					throw new ScannerException("while scanning a block scalar", startMark,
							"expected indentation indicator in the range 1-9, but found 0", this.reader.getMark());
				}
				this.reader.forward();
			}
			...
			if (!doubleQuoted && c == 39 && this.reader.peek(1) == 39) {
				...
				if (!Character.isSupplementaryCodePoint(c) && ScannerImpl.ESCAPE_REPLACEMENTS.containsKey((char) c)) {
					...
					if (this.scanLineBreak().length() == 0) {
						final String s = String.valueOf(Character.toChars(c));
						throw new ScannerException("while scanning a double-quoted scalar", startMark,
								"found unknown escape character " + s + "(" + c + ")", this.reader.getMark());
					}
		...
		while (true) {
			...
			if (("---".equals(prefix) || "...".equals(prefix)) && Constant.NULL_BL_T_LINEBR.has(this.reader.peek(3))) {
				throw new ScannerException("while scanning a quoted scalar", startMark,
						"found unexpected document separator", this.reader.getMark());
			}
		...
		do {
			...
			while (true) {
				...
				if (Constant.NULL_BL_T_LINEBR.has(c)
						|| (this.flowLevel == 0 && c == 58
								&& Constant.NULL_BL_T_LINEBR.has(this.reader.peek(length + 1)))
						|| (this.flowLevel != 0 && ",:?[]{}".indexOf(c) != -1)) {
					break;
				}
				++length;
			}
			if (this.flowLevel != 0 && c == 58
					&& Constant.NULL_BL_T_LINEBR.hasNo(this.reader.peek(length + 1), ",[]{}")) {
				this.reader.forward(length);
				throw new ScannerException("while scanning a plain scalar", startMark, "found unexpected ':'",
						this.reader.getMark(),
		...
		while (true) {
			if (this.reader.peek() == 32) {
				...
				if (lb.length() != 0) {
					...
					if ("---".equals(prefix)
							|| ("...".equals(prefix) && Constant.NULL_BL_T_LINEBR.has(this.reader.peek(3)))) {
						return "";
					}
					continue;
				} else {
	...
	private String scanTagHandle(final String name, final Mark startMark) {
		...
		if (c != 32) {
			...
			if (c != 33) {
				...
				final String s2 = String.valueOf(Character.toChars(c));
				throw new ScannerException("while scanning a " + name, startMark,
						"expected '!', but found " + s2 + "(" + c + ")", this.reader.getMark());
			}
			++length;
		}
		final String value = this.reader.prefixForward(length);
	...
	private String scanTagUri(final String name, final Mark startMark) {
		...
		if (chunks.length() == 0) {
			final String s = String.valueOf(Character.toChars(c));
			throw new ScannerException("while scanning a " + name, startMark,
					"expected URI, but found " + s + "(" + c + ")", this.reader.getMark());
		}
		return chunks.toString();
	}
