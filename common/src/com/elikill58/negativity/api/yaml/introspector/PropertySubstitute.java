...
public class PropertySubstitute extends Property {
	...
	public PropertySubstitute(final String name, final Class<?> type, final String readMethod, final String writeMethod,
			final Class<?>... params) {
		super(name, type);
	...
	public Object get(final Object object) {
		try {
			...
			throw new YAMLException(
					"Unable to find getter for property '" + this.getName() + "' on object " + object + ":" + e);
		}
	...
	public void setTargetType(final Class<?> targetType) {
		if (this.targetType != targetType) {
			...
			if (this.field == null && PropertySubstitute.log.isLoggable(Level.FINE)) {
				PropertySubstitute.log
						.fine(String.format("Failed to find field for %s.%s", targetType.getName(), this.getName()));
			}
	...
	private Method discoverMethod(final Class<?> type, final String name, final Class<?>... params) {
		...
		if (PropertySubstitute.log.isLoggable(Level.FINE)) {
			PropertySubstitute.log.fine(String.format("Failed to find [%s(%d args)] for %s.%s", name, params.length,
					this.targetType.getName(), this.getName()));
		}
		return null;
	}
