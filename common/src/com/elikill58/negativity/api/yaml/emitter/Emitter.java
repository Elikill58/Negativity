...
import com.elikill58.negativity.api.yaml.events.CollectionEndEvent;
import com.elikill58.negativity.api.yaml.events.CollectionStartEvent;
...
import com.elikill58.negativity.api.yaml.events.NodeEvent;
...
import com.elikill58.negativity.api.yaml.scanner.Constant;
...
public final class Emitter implements Emitable {
	...
	private void expectNode(final boolean root, final boolean mapping, final boolean simpleKey) throws IOException {
		...
		if (this.event instanceof AliasEvent) {
			...
			} else if (this.flowLevel != 0 || this.canonical || ((MappingStartEvent) this.event).getFlowStyle()
	...
	private boolean checkEmptySequence() {
		return this.event instanceof SequenceStartEvent && !this.events.isEmpty()
				&& this.events.peek() instanceof SequenceEndEvent;
	}

	private boolean checkEmptyMapping() {
		return this.event instanceof MappingStartEvent && !this.events.isEmpty()
				&& this.events.peek() instanceof MappingEndEvent;
	}
	...
	private boolean checkSimpleKey() {
		...
		return length < 128 && (this.event instanceof AliasEvent
				|| (this.event instanceof ScalarEvent && !this.analysis.empty && !this.analysis.multiline)
	...
	private Character chooseScalarStyle() {
		...
		if (ev.getStyle() == null && ev.getImplicit().canOmitTagInPlainScalar()
				&& (!this.simpleKeyContext || (!this.analysis.empty && !this.analysis.multiline))
				&& ((this.flowLevel != 0 && this.analysis.allowFlowPlain)
						|| (this.flowLevel == 0 && this.analysis.allowBlockPlain))) {
			return null;
		}
		if (ev.getStyle() != null && (ev.getStyle() == '|' || ev.getStyle() == '>') && this.flowLevel == 0
				&& !this.simpleKeyContext && this.analysis.allowBlock) {
			return ev.getStyle();
		}
		if ((ev.getStyle() == null || ev.getStyle() == '\'') && this.analysis.allowSingleQuoted
				&& (!this.simpleKeyContext || !this.analysis.multiline)) {
			return '\'';
		}
		return '\"';
	}
	...
	private ScalarAnalysis analyzeScalar(final String scalar) {
		...
		while (index < scalar.length()) {
			...
			if (c != 10 && (32 > c || c > 126)) {
				if (c == 133 || (c >= 160 && c <= 55295) || (c >= 57344 && c <= 65533)
						|| (c >= 65536 && c <= 1114111)) {
	...
	void writeIndicator(final String indicator, final boolean needWhitespace, final boolean whitespace,
			final boolean indentation) throws IOException {
	...
	private class ExpectDocumentStart implements EmitterState {
		...
		public void expect() throws IOException {
			if (Emitter.this.event instanceof DocumentStartEvent) {
				...
				final boolean implicit = this.first && !ev.getExplicit() && !Emitter.this.canonical
						&& ev.getVersion() == null && (ev.getTags() == null || ev.getTags().isEmpty())
	...
	private class ExpectFirstFlowSequenceItem implements EmitterState {
		...
		public void expect() throws IOException {
			if (Emitter.this.event instanceof SequenceEndEvent) {
				...
				if (Emitter.this.canonical || (Emitter.this.column > Emitter.this.bestWidth && Emitter.this.splitLines)
						|| Emitter.this.prettyFlow) {
					Emitter.this.writeIndent();
				}
