...
public class DiscordWebhook implements Webhook {
	...
	private final HashMap<WebhookMessageType, HashMap<Player, Long>> players = new HashMap<>();
	private List<WebhookMessage> queue = new ArrayList<>();
	private long time = 0, cooldown = 0;
	private boolean enabled = true;

	public DiscordWebhook(ConfigurationSection config) {
		this.config = config;
	...
	private long getTimeFor(Player p, WebhookMessageType type) {
		return players.getOrDefault(type, new HashMap<>()).getOrDefault(p, 0l);
	}
	
	private void setTimeFor(Player p, WebhookMessageType type, long time) {
		players.computeIfAbsent(type, (a) -> new HashMap<>()).put(p, time);
	}
	...
	public void setCooldown(Player p, WebhookMessageType type) {
		setTimeFor(p, type, System.currentTimeMillis() + getTheoricCooldown(type));
	}
	...
	public void addToQueue(WebhookMessage msg) {
		if (msg == null || !enabled || !msg.canBeSend(config.getConfigurationSection("messages." + msg.getMessageType().name().toLowerCase(Locale.ROOT))))
			return;
	...
	public void send(WebhookMessage msg) {
		if(!msg.canBeSend(config.getConfigurationSection("messages." + msg.getMessageType().name().toLowerCase(Locale.ROOT))))
			return;
		try {
	...
	private void sendAsyncWithException(WebhookMessage msg) throws Exception {
		ConfigurationSection confMsg = config.getConfigurationSection("messages." + msg.getMessageType().name().toLowerCase(Locale.ROOT));
		...
		ConfigurationSection embed = confMsg.getConfigurationSection("embed");
		if (embed != null) {
			...
			ConfigurationSection fields = embed.getConfigurationSection("fields");
			if (fields != null) {
				fields.getKeys(false).forEach((key) -> {
					ConfigurationSection fieldConfig = fields.getConfigurationSection(key);
