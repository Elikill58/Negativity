...
public class Inv {
	...
	public static void openCheckMenu(Player p, Player cible) {
		...
		inv.query(new SlotPos(0, 1)).set(Utils.createItem(ItemTypes.DIAMOND_SWORD, "&rFight: "
				+ Messages.getStringMessage(p, "inventory.manager." + (np.MODS.size() > 0 ? "enabled" : "disabled"))));
		inv.query(new SlotPos(1, 1))
				.set(Utils.createItem(ItemTypes.DIAMOND_PICKAXE, "&rMinerate", np.mineRate.getInventoryLoreString()));
		inv.query(new SlotPos(2, 1)).set(Utils.createItem(ItemTypes.GRASS, "&rMods", "&7Forge: "
				+ Messages.getStringMessage(p, "inventory.manager." + (np.MODS.size() > 0 ? "enabled" : "disabled"))));

		inv.query(new SlotPos(0, 2)).set(Utils.createItem(ItemTypes.SPIDER_EYE,
				Messages.getStringMessage(p, "inventory.main.see_inv", "%name%", cible.getName())));
		inv.query(new SlotPos(1, 2)).set(Utils.createItem(ItemTypes.ENDER_EYE,
				Messages.getStringMessage(p, "inventory.main.teleportation_to", "%name%", cible.getName())));
		inv.query(new SlotPos(2, 2)).set(Utils.createItem(ItemTypes.PACKED_ICE,
				Messages.getStringMessage(p, "inventory.main.freezing", "%name%", cible.getName())));
		inv.query(new SlotPos(3, 2)).set(Utils.createItem(ItemTypes.ANVIL,
				Messages.getStringMessage(p, "inventory.main.see_alerts", "%name%", cible.getName())));
		inv.query(new SlotPos(4, 2)).set(Utils.createItem(ItemTypes.TNT,
		...
		if (np.getActiveCheat().size() > 0) {
			for (Cheat c : np.getActiveCheat()) {
				if (c.equals(Cheat.ALL))
					continue;
				inv.query(new SlotPos(x, y)).set(Utils.createItem((ItemType) c.getMaterial(), "&r" + c.getName()));
				x++;
				if (x > 8) {
					x = 0;
					y++;
	...
	public static void openModMenu(Player p) {
		...
		inv.query(new SlotPos(3, 1)).set(Utils.createItem(ItemTypes.FEATHER, "&rFly: " + Messages.getStringMessage(p,
	...
	public static void openOneCheatMenu(Player p, Cheat c) {
		...
		inv.query(new SlotPos(0, 1)).set(Utils.createItem((ItemType) c.getMaterial(), c.getName()));
	...
	public static void openForgeModsMenu(Player p) {
		...
		if (np.MODS.size() == 0) {
			...
			for (Entry<String, String> entry : np.MODS.entrySet()) {
				inv.query(new SlotPos(x, y)).set(
						Utils.createItem(ItemTypes.GRASS, "&r" + entry.getKey(), "&7Version: " + entry.getValue()));
				x++;
				if (x > 8) {
					x = 0;
					y++;
