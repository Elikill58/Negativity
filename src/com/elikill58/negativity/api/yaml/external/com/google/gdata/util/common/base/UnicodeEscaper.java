...
public abstract class UnicodeEscaper implements Escaper {
			...
			public Appendable append(final CharSequence csq, final int start, final int end) throws IOException {
				...
				if (index < end) {
					...
					if (this.pendingHighSurrogate != -1) {
						...
						final char[] escaped = UnicodeEscaper.this
								.escape(Character.toCodePoint((char) this.pendingHighSurrogate, c));
						if (escaped != null) {
	...
	protected static final int codePointAt(final CharSequence seq, int index, final int end) {
		...
		if (c1 > '\udbff') {
			throw new IllegalArgumentException("Unexpected low surrogate character '" + c1 + "' with value " + (int) c1
					+ " at index " + (index - 1));
		}
		if (index == end) {
			return -c1;
