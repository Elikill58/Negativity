...
public class Composer {
	...
	public Node getSingleNode() {
		...
		if (!this.parser.checkEvent(Event.ID.StreamEnd)) {
			...
			throw new ComposerException("expected a single document in the stream", document.getStartMark(),
	...
	protected Node composeScalarNode(final String anchor) {
		...
		final Node node = new ScalarNode(nodeTag, resolved, ev.getValue(), ev.getStartMark(), ev.getEndMark(),
				ev.getStyle());
		if (anchor != null) {
	...
	protected Node composeSequenceNode(final String anchor) {
		...
		final SequenceNode node = new SequenceNode(nodeTag, resolved, children, startEvent.getStartMark(), null,
	...
	protected Node composeMappingNode(final String anchor) {
		...
		final MappingNode node = new MappingNode(nodeTag, resolved, children, startEvent.getStartMark(), null,
				startEvent.getFlowStyle());
		if (anchor != null) {
			node.setAnchor(anchor);
			this.anchors.put(anchor, node);
