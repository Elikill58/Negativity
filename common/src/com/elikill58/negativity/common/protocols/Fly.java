	...
	public void onPlayerMove(PlayerMoveEvent e) {
		...
		if(checkActive("omega-craft") && !np.isInFight) {
			...
			if(p.getFallDistance() <= 0.000001 && !p.isInsideVehicle() && onGround == wasOnGround) {
				...
				if(amount > 1) {
					Negativity.alertMod(ReportType.WARNING, p, this, UniversalUtils.parseInPorcent(90 + amount), "OmegaCraftFly - " + np.flyMoveAmount + " > " + onGround + " : " + wasOnGround, "omega-craft", (CheatHover) null, amount > 1 ? amount - 1 : 1);
				}
			}
			if((onGround && wasOnGround) || (d > 0.1 || d < -0.1) || LocationUtils.hasMaterialsAround(e.getTo(), "FENCE", "SLIME", "LILY")
					|| LocationUtils.hasMaterialsAround(locUnder, "FENCE", "SLIME", "LILY", "VINE") || hasBoatAround || p.isInsideVehicle())
		...
		if(p.hasElytra()) {
			if(checkActive("elytra")) {
				...
				if(isGoingDown) {
					...
					if(hand != null && hand.getType().equals(Materials.FIREWORK)) {
						...
						if(distance > 0.29)
							Negativity.alertMod(ReportType.WARNING, p, this, 99, "elytra", "Going UP, hand: " + hand + ". Y: " + String.format("%.5f", y) + ", distance: " + String.format("%.5f", distance) + ", velocity: " + p.getVelocity().toShowableString(), new CheatHover.Literal("Elytra fly"));
					}
				}
			}
		} else {
			...
			if(checkActive("not-moving-y")) {
				if(p.isOnGround() && y == 0 && type.equals(Materials.AIR) && locUnder.getBlock().getType().equals(Materials.AIR) && distance > p.getWalkSpeed() && !LocationUtils.hasOtherThan(loc, Materials.AIR) && !LocationUtils.hasOtherThan(locUnder, Materials.AIR)) {
					int time0 = np.ints.get(FLY, "y-0-times", 0);
					if(time0 > 2) {
						mayCancel = Negativity.alertMod(time0 > 10 ? ReportType.VIOLATION : ReportType.WARNING, p, this, parseInPorcent(time0 * 30), "not-moving-y", "Times not moving Y and on ground: " + time0 + ", distance: " + distance + ", ws: " + p.getWalkSpeed(), null, time0 < 3 ? 1 : time0 - 2);
					}
