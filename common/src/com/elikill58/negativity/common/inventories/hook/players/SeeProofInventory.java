...
public class SeeProofInventory extends AbstractInventory<SeeProofHolder> {
	...
	public void openInventory(Player p, Object... args) {
		...
		NegativityProofStorage.getStorage().getProof(cible.getUniqueId()).thenAcceptAsync(proofs -> {
			...
			if (page < 0)
				page = 0;
			Inventory inv = Inventory.createInventory("Proofs",
					UniversalUtils.getMultipleOf(proofs.size() + 9, 9, 1, 54), new SeeProofHolder(cible, page));
			for (int i = 1; i < 7; i++)
			...
			for (int i = offset; i < max; i++) {
				...
				Object[] placeholders = new Object[] { "%date%", ChatUtils.formatTime(r.getTime().getTime()),
						"%amount%", r.getAmount(), "%ping%", r.getPing(), "%report_type%", r.getReportType().getName(),
						"%check_name%", r.getCheckName(), "%reliability%", r.getReliability(), "%cheat%",
						r.getCheatKey().getName(), "%player_version%", r.getVersion().getName() };
				inv.set(slot++,
						ItemBuilder.Builder(Materials.BOOK)
								.displayName(Messages.getMessage(p, "inventory.proof.item.name", placeholders))
								.lore(Messages.getMessage(p, "inventory.proof.item.lore", placeholders)).build());
			}
			if (page > 0)
				inv.set(3, ItemBuilder.Builder(Materials.ARROW)
						.displayName(Messages.getMessage(p, "inventory.page", "%id%", page)).build());
			if (proofs.size() > max)
				inv.set(5, ItemBuilder.Builder(Materials.ARROW)
						.displayName(Messages.getMessage(p, "inventory.page", "%id%", page + 2)).build());
			p.openInventory(inv);
		});
	}
