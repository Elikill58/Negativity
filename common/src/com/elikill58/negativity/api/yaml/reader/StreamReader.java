package com.elikill58.negativity.api.yaml.reader;
...
public class StreamReader {
	...
	public static boolean isPrintable(final int c) {
		return (c >= 32 && c <= 126) || c == 9 || c == 10 || c == 13 || c == 133 || (c >= 160 && c <= 55295)
		...
		if (!this.eof) {
			...
			try {
				...
				if (converted > 0) {
					...
					final StringBuilder builder = new StringBuilder(this.buffer.length() + converted)
							.append(this.buffer).append(this.data, 0, converted);
					if (eofDetected) {
						this.eof = true;
						builder.append('\0');
					}
