...
public class SpongeBanProcessor implements BanProcessor {
	...
	private Ban toNegativityBan(Profile profile) {
		...
		return new Ban(profile.getProfile().getUniqueId(), reason, bannedBy, BanType.UNKNOW, expirationTime, null, BanStatus.ACTIVE, executionTime);
	}
	...
	private Ban toNegativityBan(org.spongepowered.api.util.ban.Ban activeBan, UUID uuid) {
		...
		return new Ban(uuid, reason, bannedBy, BanType.UNKNOW, expirationTime, null, BanStatus.ACTIVE, executionTime);
	}
}
