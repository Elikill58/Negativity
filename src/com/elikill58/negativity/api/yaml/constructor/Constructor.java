...
import com.elikill58.negativity.api.yaml.TypeDescription;
import com.elikill58.negativity.api.yaml.error.YAMLException;
import com.elikill58.negativity.api.yaml.introspector.Property;
...
public class Constructor extends SafeConstructor {
	...
	protected class ConstructMapping implements Construct {
		...
		protected Object constructJavaBean2ndStep(final MappingNode node, final Object object) {
			...
			for (final NodeTuple tuple : nodeValue) {
				...
				try {
					...
					final Property property = (memberDescription == null) ? this.getProperty(beanType, key)
					...
					Object value = (memberDescription != null) ? this.newInstance(memberDescription, key, valueNode)
							: Constructor.this.constructObject(valueNode);
		...
		private Object newInstance(final TypeDescription memberDescription, final String propertyName,
				final Node node) {
		...
		public Object construct(final Node node) {
			...
			try {
				...
				throw new ConstructorException(null, null,
						"Can't construct a java object for " + node.getTag() + "; exception=" + e2.getMessage(),
						node.getStartMark(), e2);
			}
			return result;
		}
	...
	protected class ConstructScalar extends AbstractConstruct {
		...
		private Object constructStandardJavaInstance(@SuppressWarnings("rawtypes") final Class type,
				final ScalarNode node) {
			Object result;
		...
		public Object construct(final Node node) {
			...
			if (Set.class.isAssignableFrom(node.getType())) {
				...
				if (!node.getType().isArray()) {
					final List<java.lang.reflect.Constructor<?>> possibleConstructors = new ArrayList<java.lang.reflect.Constructor<?>>(
							snode.getValue().size());
					for (final java.lang.reflect.Constructor<?> constructor : node.getType()
