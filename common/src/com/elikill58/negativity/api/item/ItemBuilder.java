...
public abstract class ItemBuilder {
	...
	/**
	 * Set the display name of the item If the display name is null, the item will
	 * take the basic minecraft name
	 * 
	...
	/**
	 * Reset display name. Equals as {@link ItemBuilder#displayName} if you set a
	 * null name
	 * 
	 * @return this builder
	 */
	...
	/**
	 * Add enchant to item without checking if it exist As essentials, this method
	 * allow you to add enchant with level at more than max allowed by default
	 * 
	 * @param enchantment the enchant name
	 * @param level       the level of the enchant
	 * @return this builder
	...
	/**
	 * Set the amount of the item. Default: 1
	 * 
	...
	/**
	 * Edit the color Work only with colorable item like leather
	 * 
	...
	/**
	 * Create an ItemBuilder with a material's name. Compatible with "type:byte" for
	 * 1.12 and less items
	 * 
	...
	/**
	 * Get the skull item showed in negativity's inventory Display name and lore are
	 * already set, and are according to the given player cible
	 * 
	...
	public static ItemStack getSkullItem(Player cible) {
		...
		return ItemBuilder.Builder(cible).displayName(cible.getName())
				.lore(ChatColor.GOLD + "UUID: " + cible.getUniqueId(),
						ChatColor.GREEN + "Version: " + cible.getPlayerVersion().getName(),
						ChatColor.GREEN + "Platform: " + (np.isBedrockPlayer() ? "Bedrock" : "Java"))
				.build();
	}
	 ...
	 * 
	 * @param cible the owner of the skull item
	 * @param who   the player that will see this skull
	 * @return the skull item which will be set in inventory
	 */
	public static ItemStack getSkullItem(Player cible, Player who) {
		NegativityPlayer np = NegativityPlayer.getNegativityPlayer(cible);
		return ItemBuilder.Builder(cible)
				.displayName(Messages.getMessage(who, "inventory.skull.name", "%name%", cible.getName()))
				.lore(Messages.getMessage(who, "inventory.skull.lore", "%name%", cible.getName(), "%uuid%",
						cible.getUniqueId().toString(), "%version%", cible.getPlayerVersion().getName(), "%platform%",
						np.isBedrockPlayer() ? "Bedrock" : "Java", "%launcher%", np.getClientName(), "%forge%",
						Messages.getMessage(who, "inventory.manager." + (np.mods.size() > 0 ? "enabled" : "disabled"))))
				.build();
	}

	/**
	 * Get the skull item for the given offline player<br>
	 * Prefer use {@link #getSkullItem(OfflinePlayer, Player)} to get it translate
	 * in good language
	 * 
	 * @param cible offline player
	...
	public static ItemStack getSkullItem(OfflinePlayer cible) {
		...
		return ItemBuilder.Builder(cible).displayName(cible.getName())
				.lore(ChatColor.RED + "Offline", ChatColor.GOLD + "UUID: " + cible.getUniqueId()).build();
	}

	/**
	 * Get the skull item for the given offline player<br>
	 * Display name and lore are already set, and are according to the given player
	 * cible
	 * 
	 * @param cible the owner of the skull item
	 * @param who   the player that will see this skull
	 * @return the skull item which will be set in inventory
	 */
	public static ItemStack getSkullItem(OfflinePlayer cible, Player who) {
		if (cible instanceof Player)
			return getSkullItem((Player) cible, who);
		return ItemBuilder.Builder(cible).displayName(cible.getName())
				.lore(ChatColor.RED + "Offline", ChatColor.GOLD + "UUID: " + cible.getUniqueId()).build();
	}
}
